# Real TIA Portal V18 Integration

Run real integration tests only on a workstation where TIA Portal V18 is installed and licensed/configured.

The V18 adapter is compiled for .NET Framework 4.8 and is intentionally isolated from the .NET 8 MCP host. Set the `TiaV18PublicApiDirectory` MSBuild property when the V18 PublicAPI directory is not installed at the default Siemens path.

`EngineerPc.Tia.V18.Worker` is started with `--configuration <absolute-path>`. Its configuration contains only locally approved `projectId` and absolute `projectFilePath` definitions. Requests contain a project ID only; the worker rejects arbitrary project paths and processes requests sequentially.

`EngineerPc.Tia.V18.Client` is the .NET 8 bridge used by the platform. Its deployment-owned options must identify an existing canonical worker `.exe`, an existing canonical worker configuration file, and a deadline between one second and five minutes. The client invokes exactly that executable with exactly the configured `--configuration` path; neither path is derived from MCP input. Ensure the deployment account restricts write access to the worker executable, its configuration, and the configured project directories.
...
```json
{
	"TiaV18Worker": {
		...
		"WorkerExecutablePath": "C:\\EngineerPc\\EngineerPc.Tia.V18.Worker.exe",
		"ConfigurationPath": "C:\\EngineerPc\\tia-v18-worker.json",
	...
	"projects": [
		{
			...
			"projectFilePath": "C:\\Engineering\\TestProject.ap18"
		}
	]
}
```

The installed Siemens V18 PublicAPI directory defaults to `C:\Program Files\Siemens\Automation\Portal V18\PublicAPI\V18`. Set the deployment-owned environment variable `TIA_V18_PUBLIC_API_DIRECTORY` only when the V18 PublicAPI has been installed elsewhere. Do not put this directory in worker request data.
...
1. Verify V18 project-context reads using a configured local project ID and file path.
